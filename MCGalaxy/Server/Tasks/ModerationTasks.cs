...
namespace MCGalaxy.Tasks {
    internal static class ModerationTasks {
        ...
        static SchedulerTask temprankTask, freezeTask, muteTask, jailTask;
    ...
    JailCheckTask, null, NextRun(Server.jailed));
        ...
        internal static void JailCheckTask(SchedulerTask task)
        {
            DoTask(task, Server.jailed, JailCallback);
        }

        internal static void JailCalcNextRun() { CalcNextRun(jailTask, Server.jailed); }

        static void JailCallback(string[] args)
        {
            ModAction action = new ModAction(args[0], Player.Console, ModActionType.Unjailed, "auto unjail");
            OnModActionEvent.Call(action);
        }
